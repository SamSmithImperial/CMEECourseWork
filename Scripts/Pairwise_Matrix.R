...
library(parallel)
data <- read.csv("../data/Other/sequences123.csv")
data$barcode_data <- substring(data$Sequence, 261, 937) # Extract long region of COX1
data$barcode_data <- lapply(data$barcode_data, function(x) strsplit(x, "")[[1]]) # Convert Character string sequences into vectors

data$barcode_data_c1 <- lapply(data$barcode_data, function(sequence) sequence[seq(1, length(sequence), by = 3)]) ##
data$barcode_data_c2 <- lapply(data$barcode_data, function(sequence) sequence[seq(2, length(sequence), by = 3)]) ### This separates into codon positions
data$barcode_data_c3 <- lapply(data$barcode_data, function(sequence) sequence[seq(3, length(sequence), by = 3)]) ##
